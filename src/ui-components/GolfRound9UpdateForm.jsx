...
export default function GolfRound9UpdateForm(props) {
  const {
    id: idProp,
    golfRound9,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = golfRound9Record
      ? { ...initialValues, ...golfRound9Record }
      : initialValues;
  ...
  const [golfRound9Record, setGolfRound9Record] = React.useState(golfRound9);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(GolfRound9, idProp)
        : golfRound9;
      setGolfRound9Record(record);
    };
    queryData();
  }, [idProp, golfRound9]);
  React.useEffect(resetStateValues, [golfRound9Record]);
  const validations = {
    date: [{ type: "Required" }],
    hole1: [],
    hole2: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            GolfRound9.copyOf(golfRound9Record, (updated) => {
      ...
      {...getOverrideProps(overrides, "GolfRound9UpdateForm")}
      {...rest}
    >
      <TextField
        label="Date"
